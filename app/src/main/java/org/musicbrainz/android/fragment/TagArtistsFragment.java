...
public class TagArtistsFragment extends Fragment implements
        RetryCallback {
    ...
    private PagedArtistTagAdapter adapter;
    ...
    public static TagArtistsFragment newInstance() {
        Bundle args = new Bundle();
        TagArtistsFragment fragment = new TagArtistsFragment();
    ...
    public void load() {
        ...
        if (tag != null) {
            adapter = new PagedArtistTagAdapter(this);
            adapter.setHolderClickListener(tagEntity -> ((OnArtistCommunicator) getContext()).onArtist(tagEntity.getMbid()));

            tagViewModel = ViewModelProviders.of(this).get(TagViewModel.class);
            tagViewModel.load(ARTIST, tag);
